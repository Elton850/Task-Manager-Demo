    ...
    <div className="flex flex-col gap-1">
      ...
      <div className="relative">
        <select
          ...
          className={`
            w-full rounded-lg bg-white dark:bg-slate-700/90 border text-slate-900 dark:text-slate-100
      ...
      {error && <p id={`${selectId}-error`} className="text-xs text-rose-500 dark:text-rose-400" role="alert">{error}</p>}
      {hint && !error && <p id={`${selectId}-hint`} className="text-xs text-slate-500 dark:text-slate-400">{hint}</p>}
    </div>
  );
}
